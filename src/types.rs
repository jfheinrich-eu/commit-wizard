...
mod tests {
    ...
    fn test_change_group_header_truncation() {
        let group = ChangeGroup::new(
            ...
            "This is a very long description that should be truncated to fit within the maximum header length limit".to_string(),
            vec![],
        );
    ...
    #[test]
    fn test_change_group_full_message() {
        let group = ChangeGroup::new(
            CommitType::Fix,
            Some("api".to_string()),
            vec![],
            ...
            vec!["add null checks".to_string(), "update tests".to_string()],
        );
